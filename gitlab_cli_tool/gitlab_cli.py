...
class GitLabCLI:
    ...
    @staticmethod
    def parse_args(args):
        parser = argparse.ArgumentParser(description="CLI for GitLab, to exit CTRL + D")

        parser.add_argument(
            "property_name",
            help="Projects, Runners, etc",
        ...
        parser.add_argument(
            "action",
            help="What to do with property, list, pause, resume",
            default="list",
            nargs="*",
        )
        ...
        parser.add_argument(
            "-v",
            "--variables",
            help="Triggering branch with variables, format key=value",
            nargs="+",
        )
        parser.add_argument(
            "-i",
    ...
    def assign_args_to_cli(self, args):
        ...
        self.branch = (
            parsed_args.branch[0] if parsed_args.branch else parsed_args.branch
        )
